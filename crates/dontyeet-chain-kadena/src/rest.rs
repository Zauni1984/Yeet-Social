//! Internal REST/Pact API helper for Kadena.
//!
//! Provides thin wrappers for Chainweb Pact API calls using the first
//! available URL from a list.
...
#[expect(
    dead_code,
    reason = "GET sibling to rest_post; kept for future read endpoints (Pact local calls)"
)]
pub async fn rest_get<T: DeserializeOwned>(urls: &[Url], path: &str) -> Result<T> {
    if urls.is_empty() {
        return Err(DontYeetWalletError::Network(
            "no API URLs configured for this Kadena network".into(),
        ));
    }
