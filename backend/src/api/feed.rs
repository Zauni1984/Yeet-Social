...
pub async fn get_feed(
    ...
    let rows = sqlx::query_as::<_, FeedRow>(
        "SELECT p.id, p.content, p.media_url, p.is_adult,
         ...
         WHERE p.expires_at > NOW() AND ($1 OR p.is_adult = false)
         ORDER BY p.created_at DESC LIMIT $2 OFFSET $3"
    )
    .bind(show_adult).bind(per_page).bind(offset)
...
pub async fn get_following_feed(
    ...
    let rows = sqlx::query_as::<_, FeedRow>(
        "SELECT p.id, p.content, p.media_url, p.is_adult,
         ...
         WHERE f.follower_id = $1 AND p.expires_at > NOW() AND ($2 OR p.is_adult = false)
         ORDER BY p.created_at DESC LIMIT $3 OFFSET $4"
    )
    .bind(user_id).bind(show_adult).bind(per_page).bind(offset)
    .fetch_all(state.db.pool()).await.map_err(AppError::Database)?;
...
fn row_to_feed_post(r: FeedRow) -> FeedPost {
    FeedPost {
        ...
        is_nft: r.nft_token_id.is_some(), like_count: r.like_count,
        reshare_count: r.reshare_count, comment_count: r.comment_count,
