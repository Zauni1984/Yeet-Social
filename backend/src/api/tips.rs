...
pub struct SendTipRequest {
    pub to_address: String, pub amount: String, pub currency: String,
    pub post_id: Option<Uuid>, pub tx_hash: Option<String>,
}
...
pub async fn send_tip(
    ...
    let from_id: Uuid = sqlx::query_scalar("SELECT id FROM users WHERE wallet_address = $1")
    ...
    let to_id: Uuid = sqlx::query_scalar("SELECT id FROM users WHERE wallet_address = $1")
        .bind(req.to_address.to_lowercase()).fetch_optional(state.db.pool()).await.map_err(AppError::Database)?
        .ok_or_else(|| AppError::NotFound("Recipient not found".into()))?;
    if from_id == to_id { return Err(AppError::Validation("Cannot tip yourself".into())); }
    ...
    .bind(from_id).bind(to_id).bind(req.post_id)
    .bind(&req.amount).bind(&req.currency).bind(&req.tx_hash)
